...
def generate_clients_csv():
    try:
        ...
        # Передавать потом название файла в качестве параметра
        # Посмотреть xcom
        # Запускается даг с созданием файла
        # После запускает сенсор S3 когда создался файл
        # Потом запускается скрипт по загрузке данных в клик
        filename = f"/opt/airflow/dags/Scripts/Temp/{data}.csv"
        ...
        print(f"Ошибка в работе метода generate_clients: {e}")
